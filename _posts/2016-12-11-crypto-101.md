...
Also this is very light an CTF-specific. If you are interested in Crypto check out [cypto101.io](https://cypto101.io)**  
...
#### EBC Mode
![EBC-ENC](https://upload.wikimedia.org/wikipedia/commons/thumb/d/d6/ECB_encryption.svg/601px-ECB_encryption.svg.png){: .center-image .threequarter-image }
![EBC-DEC](https://upload.wikimedia.org/wikipedia/commons/thumb/e/e6/ECB_decryption.svg/601px-ECB_decryption.svg.png){: .center-image .threequarter-image }

[EBC](https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation#Electronic_Codebook_.28ECB.29) is the basic of block cipher mode. Every block of plaintext is encrypted with the same key.  
...
e.g. In this [ABCTF2016 Challenge](https://gist.github.com/bobacadodl/a3c11057d81d81b4b7c941c6adbf36d5), you are given a bunch of AES-128 EBC Plaintext-Ciphertext pairs, and the encrypted flag `e220eb994c8fc16388dbd60a969d4953f042fc0bce25dbef573cf522636a1ba3fafa1a7c21ff824a5824c5dc4a376e75`  
...
e.g. Let's use AES-128 EBC that has `16 byte` blocks  
We send the garbage message lenght `15` -> `AAAAAAAAAAAAAAA`  
...
There is a [script]({{ site.url }}/assets/crypto_ebc_eoracle.py) that do this for the [EncryptionService - ABCTF2016](https://kimiyuki.net/blog/2016/07/23/abctf-2016/) challenge
...
You have a ciphertext `C1` and it's IV `IV1`, you want know if that is the encryption of the plaintext `P1`.  
If you can predict the next IV (`IV2`), you can made up your plaintext like: `P2 = P1 ^ IV1 ^ IV2` (where `^` is the XOR operation).  
